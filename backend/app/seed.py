...
"""
    ...
    for proj in projects_list:
        ...
        if not existing:
            ...
            db.add(db_proj)
            db.commit()
            print(f"Baza danych: Pomyślnie zasiano projekt {proj['title']}!")
        else:
            # Aktualizujemy istniejący projekt nowymi danymi z kodu
            existing.objective = proj["objective"]
            existing.method = proj["method"]
            existing.strike_team = proj.get("strike_team", "")
            existing.hardware_mod = proj.get("hardware_mod", "")
            existing.tech_stack = proj["tech_stack"]
            existing.source_code = proj["source_code"]
            existing.live_link = proj["live_link"]
            existing.image_url = proj["image_url"]
            existing.status = proj["status"]
            existing.view_logs = proj["view_logs"]
            db.commit()
            print(f"Baza danych: Pomyślnie zaktualizowano istniejący projekt {proj['title']}!")
